...
import shared.plotly.{Bar, Box, Scatter}
...
object SampleDetail {
  ...
  def refreshStrDetailTable(filterRs: js.Array[js.Dictionary[String]], f: () => js.Any = () => ()) = {
    val tableId = "strDetailTable"
    $(s"#${tableId}").bootstrapTable("load", filterRs)
    f()
  }
      ...
      success { (data: js.Any, status: String, e: JQueryXHR) =>
        ...
        val html =
          rs.map { case dict =>
            ...
            //              table(cls := " table table-bordered strTable", width := 180,
            //                tr(height := first1Height, maxHeight := first1Height, minHeight := first1Height,
            //                  td(colspan := 2, key, fontWeight := "bold", backgroundColor := "#5E738B", paddingTop := 3, paddingBottom := 3)
            //                ),
            //                tr(height := first1Height, maxHeight := first1Height, minHeight := first1Height,
  ...
  def strDetailShow(filterRs: js.Array[js.Dictionary[String]]) = {
    ...
    $(".locus").text(locus)
    refreshStrDetailTable(filterRs)
    strDetailPlot(filterRs)

    jQuery("#locusModal").modal("show")
  }

  def fillReadsData(rs: js.Array[js.Dictionary[String]], seqData: js.Array[js.Dictionary[String]], divId: String) = {
  ...
  def autoShow(data: js.Array[js.Dictionary[String]], seqData: js.Array[js.Dictionary[String]]) = {
    ...
    scatterPlot(filterArray, "autoScatterChart")
    fillReadsData(filterArray, seqData, "autoData")
  ...
  def yShow(data: js.Array[js.Dictionary[String]], seqData: js.Array[js.Dictionary[String]]) = {
    ...
    val parentId="yStr"
    barPlot(filterArray,  $(s"#${parentId} #barChart"))
    scatterPlot(filterArray, "yScatterChart")
    fillReadsData(filterArray, seqData, "yData")
    val ts = filterArray.map { dict =>
      (dict(locusStr), dict(genotypeStr))
    }
    val filterSeqData = seqData.filter { dict =>
  ...
  def barPlot(filterArray: js.Array[js.Dictionary[String]], jq: JQuery) = {
    ...
    val config = PlotlyConfigOptions.displayModeBar(true)
  ...
  def scatterPlot(rs: js.Array[js.Dictionary[String]], chartId: String) = {
    val xs = rs.map { dict =>
      dict("ProductSize")
    }.toList
